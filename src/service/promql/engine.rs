...
impl Engine {
    ...
    async fn aggregate_exprs(
        ...
        Ok(match op.id() {
            ...
            token::T_TOPK => aggregations::topk(self, param.clone().unwrap(), modifier, &input).await?,
            token::T_BOTTOMK => aggregations::bottomk(self, param.clone().unwrap(), modifier, &input).await?,
